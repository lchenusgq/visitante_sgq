...
import sys
import psycopg2
#from psycopg2 import register_type, UNICODE
...
def reporte_visitas_pdf(request):
    ...
    #ruta_pdf = os.path.join(settings.STATICFILES_DIRS[0], 'pdfs', 'visitas.pdf')

    ruta_pdf = os.path.join(settings.BASE_DIR, 'registro_visitantes', 'static', 'pdfs', 'visitas.pdf')
    ...
    if request.method == 'POST':
        ...
        visitas = RegistroVisita.objects.filter(fecha_hora__date__range=[fecha_inicio, fecha_fin]).order_by('fecha_hora')
        #print(visitas.query)  # Imprime la consulta SQL generada
        ## fin pruebas
        ...
        # Define un estilo para el párrafo que permita el ajuste de línea
        estilo_parrafo = ParagraphStyle(
            name='CustomStyle',
            fontSize=9,
            leading=12,
            wordWrap='CJK'  # Permite el ajuste de línea
        )
        ...
        #logo_path = "//mnt/visitante_sgq//registro_visitantes//static//images//logoSGQ.png"    ## configuracion para linux ##
        ...
        for visita in visitas:
            ...
            nombre_completo = f"{visitante.nombres} {visitante.apellidos}"
            data.append([
                fecha_hora_local.strftime('%d/%m/%Y'),
                fecha_hora_local.strftime('%H:%M'),
                Paragraph(nombre_completo, estilo_parrafo),  # Usar Paragraph para nombre y apellido
        ...
        # Ajustar las proporciones de las columnas para reducir el espacio de la fecha
        proporciones = [0.10, 0.08, 0.30, 0.12, 0.15, 0.25]  # Reducir la proporción de la columna de la fecha
        #proporciones = [0.10, 0.10, 0.25, 0.15, 0.15, 0.25]  # Reducir la proporción de la columna de la fecha
...
def descargar_pdf(request):
    ...
    ruta_pdf = os.path.join(settings.BASE_DIR, 'registro_visitantes', 'static', 'pdfs', 'visitas.pdf')
    #ruta_pdf="//mnt/visitante_sgq//registro_visitantes//static//pdfs"  ## configuracin para linux ##
...
def login(request):
    ...
    #return render(request, 'registro_visitantes//login.html')  ## configuracion para linux ##
...
def registro_visitante(request):
    ...
    #return render(request, 'registro_visitantes//registro_visitante.html', {'oficinas': oficinas, 'fecha_hora': fecha_hora_actual})  ## configuracion para linux ##
...
def registro_visitante(request):
    ...
    if request.method == 'POST':
        ...
        visitante_db = None
        #print("....pase por aqui.....")
        # Paso 1: Buscar en la base local
        try:
        ...
        # Paso 2: Si no existe en la base local o le faltan datos, buscar en personasDB
        if visitante is None or not visitante.nombres or not visitante.apellidos:
            with connections['personasDB'].cursor() as cursor:
                cursor.execute("""
                    SELECT nombres, apellidos FROM ciudadano WHERE cedula = %s
                """, [cedula])
                row = cursor.fetchone()
                
            #print("....pase por aqui.....")
            if row:
                nombres_db, apellidos_db = row
                if visitante is None:
                    # Crear visitante si no existía en la base local
                    visitante = Visitante.objects.create(
                        cedula=cedula,
                        nombres=nombres_db,
                        apellidos=apellidos_db
                    )
                    visitante_db = 'personasDB'
                else:
                    # Completar datos si estaban vacíos
                    visitante.nombres = nombres_db
                    visitante.apellidos = apellidos_db
                    visitante.save()
                    visitante_db = 'combinado'
            else:
                # Si tampoco se encuentra en personasDB, verificar que se hayan ingresado manualmente
                if not nombres or not apellidos:
                    messages.error(request, "No se encontraron los datos. Ingrese los datos manualmente.")
                    return redirect('registro_visitante')
        ...
        messages.success(
            request,
            f"Visita registrada con éxito para {visitante.nombres} {visitante.apellidos} (origen: {visitante_db})."
        )
        return redirect('registro_visitante')

    fecha_hora_actual = timezone.now()
    return render(request, 'registro_visitantes/registro_visitante.html', {
        'oficinas': oficinas,
...
def verificar_cedula(request):
    if request.method == 'POST':
        try:
            ...
            # 1. Buscar en visitante (asumiendo UTF-8)
            with connections['default'].cursor() as cursor:     
                cursor.execute("SELECT nombres, apellidos FROM registro_visitantes_visitante WHERE cedula = %s", [cedula])
                visitante = cursor.fetchone()
                if visitante:
                    print("🔎 Resultado en visitante:", visitante)
                    return JsonResponse({'exists': True, 'nombres': visitante[0], 'apellidos': visitante[1]})

            # 2. Buscar en ciudadano (manejo cuidadoso de codificación)
            with connections['personasDB'].cursor() as cursor:
                cursor.execute("""
                ...
                print("🔎 Resultado en ciudadano:", ciudadano)

                if ciudadano:
                    nombres, apellidos = ciudadano

                    # Verificamos si son memoryview (por bytea)
                    if isinstance(nombres, memoryview):
                        nombres = nombres.tobytes().decode('latin1')  # Cambia a utf-8 si corresponde
                    if isinstance(apellidos, memoryview):
                        apellidos = apellidos.tobytes().decode('latin1')

                    # Si son bytes directamente
                    if isinstance(nombres, bytes):
                        nombres = nombres.decode('latin1')
                    if isinstance(apellidos, bytes):
                        apellidos = apellidos.decode('latin1')

                    # Si ya son str, los dejamos así

                    return JsonResponse({'exists': True, 'nombres': nombres, 'apellidos': apellidos})

            print("⚠️ Cédula no encontrada en ninguna base de datos")
            return JsonResponse({'exists': False})

        except Exception as e:
            print("❌ Error al consultar ciudadano:", e)
...
""""    ## este funciona en Linux ##
def ver_pdf(request):
    ruta_pdf = os.path.join(settings.BASE_DIR, 'registro_visitantes', 'static', 'pdfs', 'visitas.pdf')
    #ruta_pdf = "F:/ProyectosDjango/visitante_sgq/registro_visitantes/static/pdfs/visitas.pdf'"   ## configuracion para linux ##
    #ruta_pdf = "//mnt/visitante_sgq//registro_visitantes//static//pdfs//visitas.pdf"   ## configuracion para linux ##

    print(f"ruta pdf en ver_pdf: {ruta_pdf}")

    if os.path.exists(ruta_pdf):
        with open(ruta_pdf, 'rb') as pdf:
            response = HttpResponse(pdf.read(), content_type='application/pdf')
            response['Content-Disposition'] = 'inline; filename="visitas.pdf"'
            response['X-Frame-Options'] = 'ALLOWALL'  # Permitir que el PDF se cargue en un iframe
            return response
    else:
        return HttpResponse("El archivo no existe.", status=404)
"""
...
def ver_pdf(request):
    ruta_pdf = "F:/ProyectosDjango/visitante_sgq/registro_visitantes/static/pdfs/visitas.pdf"
    if os.path.exists(ruta_pdf):
